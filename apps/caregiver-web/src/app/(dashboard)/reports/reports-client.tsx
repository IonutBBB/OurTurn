              ...
              <div className="divide-y divide-surface-border">
                {reports.map((report) => (
                  <button
                    ...
                    className={`w-full text-left p-4 hover:bg-brand-50 dark:hover:bg-surface-elevated transition-colors ${
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {Object.entries(content.activity_completion.by_category).map(([cat, rate]) => (
                      <div key={cat} className="card-inset rounded-2xl p-3">
                        <p className="text-xs text-text-muted">{getCategoryLabel(cat)}</p>
                        <p className="text-lg font-display font-bold text-text-primary">{rate}%</p>
                      </div>
                    ))}
                  </div>
                </div>
