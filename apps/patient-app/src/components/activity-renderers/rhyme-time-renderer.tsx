...
import type { RhymeTimeContent } from '../../data/bundled-activities';

type FeedbackState = 'none' | 'correct' | 'incorrect';

export default function RhymeTimeRenderer({
  content,
  onComplete,
  onSkip,
}: ActivityRendererProps) {
  const { t } = useTranslation();
  const data = content as RhymeTimeContent;
  ...
  const handleSelect = useCallback(async (index: number) => {
    ...
    setTimeout(() => {
      if (roundIndex + 1 >= totalRounds) {
        onComplete({ activity: 'rhyme_time', roundsCompleted: totalRounds });
      } else {
  ...
  const handleDone = useCallback(async () => {
    await Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    onComplete({ activity: 'rhyme_time', roundsCompleted: roundIndex + 1 });
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.instruction}>
        {t('patientApp.stim.rhymeTime.instruction')}
      </Text>
      ...
      {feedback === 'incorrect' && (
        <View style={styles.feedbackCardTryAgain}>
          <Text style={styles.feedbackTextTryAgain}>
            {t('patientApp.stim.rhymeTime.notQuite', { answer: current.options[current.correctIndex] })}
          </Text>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  wordCard: {
    ...
    borderColor: COLORS.cognitive, marginBottom: 24, width: '100%', alignItems: 'center',
  },
  wordText: {
    fontSize: 36, fontFamily: FONTS.display, color: COLORS.cognitive, textAlign: 'center',
  },
