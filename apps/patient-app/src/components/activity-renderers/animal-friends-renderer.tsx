...
import type { AnimalFriendsContent } from '../../data/bundled-activities';

export default function AnimalFriendsRenderer({
  content,
  onComplete,
  onSkip,
}: ActivityRendererProps) {
  const { t } = useTranslation();
  const data = content as AnimalFriendsContent;

  if (!data) return null;

  const handleDone = async () => {
    await Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    onComplete({ activity: 'animal_friends' });
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.label}>
        {t('patientApp.stim.animalFriends.didYouKnow')}
      </Text>
